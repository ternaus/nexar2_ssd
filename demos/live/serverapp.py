        ...
        while True:
            ...
            if a != -1 and b != -1:
                ...
                i = cv2.imdecode(np.fromstring(
                    jpg, dtype=np.uint8), cv2.CV_LOAD_IMAGE_COLOR)
                if count % 100 == 0:
                    res = predict(i)
                    color = (0, 255, 0)
                    cv2.rectangle(image, (res[0], res[1]), (res[
                                  2] - res[0] + 1, res[3] - res[1] + 1), color, 2)
                    font = cv2.FONT_HERSHEY_SIMPLEX
                    cv2.putText(image, res[4], (res[0], res[
                                1]), font, 4, (255, 255, 255), 2, cv2.LINE_AA)
